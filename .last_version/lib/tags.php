...
class TagsTable extends Entity\DataManager
{
	 ...
	 * @param mixed $param строка (сразделитель запятая) или массив тегов
     * @return array
	 ...
	 * - FILTER - фильтр для дополнительной фильрации выборки. Обязтеьно дополняется TAGS и SITE_ID
	 * - SORT - Массив, содержащий признак сортировки в виде наборов "название поля"=>"направление".
	 * - MIN
	 * -MAX
	 * @return array
     * @throws SystemException В случае отсутсвия модуля поиска
     * @throws \Bitrix\Main\ArgumentException
...
// 						LocalRedirect($url, false, '301 Moved permanently');
				...
				while ($arStat = $obSearch->Fetch()) {
					...
					if ($arStat['CNT'] > $max) $max = $arStat['CNT'];
					$all += $arStat['CNT'];
                 ...
                 * @var array $arStat
                 */
                foreach ($arResult['CLOUD'] as $i => &$arStat) {
	...
	{
		$arFields['DATEUPDATE'] = new \Bitrix\Main\Type\DateTime();
