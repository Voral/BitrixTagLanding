...
if ($POST_RIGHT === "D" || !Loader::includeModule('vasoft.tags')) {
...
$ID = (int)$ID;        // идентификатор редактируемой записи
...
// ******************************************************************** //
$lastError = '';
$needSave = isset($save) && $save !== '';
$needApply = isset($apply) && $apply !== '';

if (
    $REQUEST_METHOD === "POST" // проверка метода вызова страницы
    && ($needSave || $needApply) // проверка нажатия кнопок "Сохранить" и "Применить"
    ...
    if ($res) {
        ...
        if ($e = $APPLICATION->GetException()) {
            // если в процессе сохранения возникли ошибки - получаем текст ошибки и меняем выше определённые переменные
...
?>
    <form method="POST" Action="<?= $APPLICATION->GetCurPage() ?>" ENCTYPE="multipart/form-data" name="post_form">
        ...
        <tr>
            ...
            <td width="60%"><input type="text" name="PHRASE" value="<?= $str_PHRASE ?>" size="30" maxlength="100"
                                   required></td>
        </tr>
        <tr>
            <td width="40%"><? echo Loc::getMessage("VASOFT_TAGS_FIELD_CODE"); ?><span class="required">*</span></td>
            <td width="60%"><input type="text" name="CODE" value="<?= $str_CODE ?>" size="30" maxlength="100" required>
            </td>
        </tr>
        <tr>
            <td width="40%"><? echo Loc::getMessage("VASOFT_TAGS_FIELD_TITLE"); ?></td>
            <td width="60%"><input type="text" name="TITLE" value="<?= $str_TITLE ?>" size="30" maxlength="250"></td>
        </tr>
        <tr>
            <td width="40%"><? echo Loc::getMessage("VASOFT_TAGS_FIELD_BROWSER_TITLE"); ?></td>
            <td width="60%"><input type="text" name="BROWSER_TITLE" value="<?= $str_BROWSER_TITLE ?>" size="30"
                                   maxlength="255"></td>
        </tr>
        <tr>
            <td width="40%"><? echo Loc::getMessage("VASOFT_TAGS_FIELD_EYWORDS"); ?></td>
            <td width="60%"><input type="text" name="KEYWORDS" value="<?= $str_KEYWORDS ?>" size="30" maxlength="255">
            </td>
        </tr>
        <tr>
            <td width="40%"><? echo Loc::getMessage("VASOFT_TAGS_FIELD_DESCRIPTION"); ?></td>
            <td width="60%"><textarea name="DESCRIPTION" cols="60" row="10"><?= $str_DESCRIPTION ?></textarea></td>
        </tr>
        <tr>
            <td colspan="2">
                <?= Loc::getMessage("VASOFT_TAGS_FIELD_TEXT") ?>
